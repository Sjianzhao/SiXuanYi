...
uint8_t SendBuff[SENDBUFF_SIZE]; //等发送的数据
...
//  SPI_FLASH_CS_HIGH();      //GPIO_SetBits(GPIOA, GPIO_Pin_4) 拉高A4口
	GPIO_SetBits(GPIOA, GPIO_Pin_4);// 拉高A4口
...
void USART3_Config(void)
{
		...
		GPIO_PinRemapConfig(GPIO_PartialRemap_USART3,ENABLE); //映射
...
void send_byte_USART1(u8 ch)//发送一个字节数据
{
		USART_SendData(USART1, (uint8_t) ch);
...
void send_USART1(unsigned char *DataToSend , u8 data_num) //发送一串数据
{	
	u8 i;
